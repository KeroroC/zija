...
- **回收站附件可被成员立即永久删除，跳过保留期。** 保留期是便利性默认而非硬承诺：用户对敏感或错传的文件有即时清除诉求。永久删除同时清除文件卷对象与数据库行，不可恢复；记审计 `FILE_PURGED`，不发公开领域事件（封面指定在附件进回收站时已清除，无消费者需要感知物理清除）。权限与回收/恢复同级（任何成员）。
...
- **只靠保留期届满自动清除、不提供主动永久删除：** 更安全，但敏感或错传的文件在保留期内无法彻底清除，用户诉求无法满足。
...
- 永久删除 UI 置于回收站视图，需不可恢复确认（含「备份不再包含」提示）；非回收站文件拒绝永久删除。与「恢复」并发时以行级 `UPDATE ... WHERE deleted_at IS NOT NULL` 判定：恢复先赢则本次永久删除返回冲突。
