...
家庭、物品、批次都可以挂附件（说明书、小票、证件扫描件等），一份附件带名字、挂载点与回收站状态；封面是物品上某份合格图片附件的指定，不是第二种文件（见 ADR-019）。

- 允许类型封闭：JPEG、PNG、WebP、HEIC（含 HEIF）、PDF、Markdown、TXT、Word / PowerPoint / Excel（含旧版 .doc / .ppt / .xls）；拒绝压缩包等未点名类型；
- 图片单文件上限 5 MiB，文档 20 MiB；后端校验声明类型与实际内容一致；
- 封面仅 JPEG / PNG / WebP 可指定；同一物品至多一个封面；
- 文件使用系统生成名称，原始名称作为附件名字（可改名）；
- 删除附件进入回收站，保留期满后由清除任务物理删除；文件卷与数据库处于同一备份策略，卷内含回收站中尚未物理删除的对象；
- 完整性报告继续核对待物理删除的对象（含回收站）。
