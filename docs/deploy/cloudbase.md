...
端口分配：**nginx = 8080（对外）**，**后端 = 8081（仅容器内）**。后端必须让出 8080，否则会和 nginx 抢端口，表现为服务 502、日志 `Port 8080 was already in use`。

---

## ⚠️ 镜像必须是 linux/amd64

CloudBase 云托管节点是 **linux/amd64**。在 Apple Silicon（M 系列，arm64）的 Mac 上直接 `docker build` 会产出**只有 arm64 的镜像**，推上去后 pod 拉取失败：
...
> manifest 里出现的 `unknown/unknown` 条目是构建证明（attestation），不是可运行平台。

---

## 方式 A：云端构建（推荐）

把源码交给 CloudBase，平台在 amd64 机器上构建，从根本上避免架构问题。

1. 确认目标环境已**开通云托管**（新环境需单独开通一次）。
2. 通过 **CLI / MCP** 部署。链路把仓库作为上下文上传，用参数指定 Dockerfile 路径：
   ...
   > 「未配置 VPC」提示可忽略——数据库在容器内部（`127.0.0.1:5432`），不走 VPC。

3. 控制台表单式构建也可走：`Dockerfile.cloudbase` 已在仓库根，与「目标目录 = `.`」同级，控制台「Dockerfile 名称」填 `Dockerfile.cloudbase` 即可。

---

## 方式 B：本地交叉构建 amd64 并推送

不方便走云端构建时使用。**必须**显式指定平台。
...
- `--platform linux/amd64` 是核心参数，缺了会复现 `ImagePullBackOff`。
- `--provenance=false` 去掉 `unknown/unknown` 的 attestation manifest，避免部分运行时解析异常。
- 末尾 `.` 构建上下文必须是**仓库根目录**。
- Apple Silicon 走 QEMU 模拟 amd64，镜像内含 npm build 与 Maven package，**耗时通常 20–40 分钟**。
- 建议用递增版本号代替 `latest`（如 `20260731-1`）。`latest` 容易被节点缓存。
- 推送完成后回控制台**更新服务镜像版本并重新发布**。
...
| 变量 | 值 | 说明 |
|---|---|---|
| `ZIJA_DB_URL` | `jdbc:postgresql://127.0.0.1:5432/zija` | 指向**容器内自建** PostgreSQL |
...
| `ZIJA_AI_EMBEDDING_MODEL` | `qwen3-embedding:0.6b` | 可选 embedding 模型，必须输出 1024 维 |
...
容器内持久化目录统一在 `/var/lib/zija/` 下：PostgreSQL 数据为 `/var/lib/zija/postgresql/data`，上传文件为 `/var/lib/zija/files`。
...
如需对抗后两种情况，在服务配置的 `VolumesConf` 中把 CFS 文件存储卷挂载到 `/var/lib/zija/postgresql/data` 与 `/var/lib/zija/files`（两个挂载点同源时也可共用一份卷）。
...
| 中文响应变成 `????` | Servlet 响应默认 ISO-8859-1 | 已通过 `setCharacterEncoding(UTF_8)` + `server.servlet.encoding.force=true` 修复 |
