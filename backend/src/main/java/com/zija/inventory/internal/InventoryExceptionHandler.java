...
class InventoryExceptionHandler {
    ...
    ProblemDetail handleInsufficientStock(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "库存不足", "INVENTORY_INSUFFICIENT_STOCK");
    }
    ...
    ProblemDetail handleQuantityPrecisionInvalid(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.UNPROCESSABLE_CONTENT, "数量精度超过单位允许范围", "INVENTORY_QUANTITY_PRECISION_INVALID");
    }
    ...
    ProblemDetail handleIdempotencyConflict(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "幂等键冲突", "INVENTORY_IDEMPOTENCY_CONFLICT");
    }

    @ExceptionHandler(InventoryArchivedItemException.class)
    ProblemDetail handleArchivedItem(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "归档物品不可操作", "INVENTORY_ARCHIVED_ITEM");
    }
    ...
    ProblemDetail handleLotVersionConflict(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "批次版本冲突", "INVENTORY_LOT_VERSION_CONFLICT");
    }

    @ExceptionHandler(InventoryLotNotFoundException.class)
    ProblemDetail handleLotNotFound(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.NOT_FOUND, "批次不存在", "INVENTORY_LOT_NOT_FOUND");
    }
    ...
    ProblemDetail handleMovementAlreadyReversed(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "流水已撤销", "INVENTORY_MOVEMENT_ALREADY_REVERSED");
    }
    ...
    ProblemDetail handleReversalNotAllowed(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "该类型流水不允许撤销", "INVENTORY_REVERSAL_NOT_ALLOWED");
    }
    ...
    ProblemDetail handleReversalWouldNegative(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "撤销会导致库存为负", "INVENTORY_REVERSAL_WOULD_NEGATIVE");
    }

    @ExceptionHandler(StocktakeStaleException.class)
    ProblemDetail handleStocktakeStale(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "盘点范围内库存已变化", "INVENTORY_STOCKTAKE_STALE");
    }

    @ExceptionHandler(StocktakeNotDraftException.class)
    ProblemDetail handleStocktakeNotDraft(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "盘点单不是草稿状态", "INVENTORY_STOCKTAKE_NOT_DRAFT");
    }

    @ExceptionHandler(IllegalArgumentException.class)
    ProblemDetail handleIllegalArgument(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.BAD_REQUEST, "请求字段校验失败", "VALIDATION_FAILED");
    }
}
