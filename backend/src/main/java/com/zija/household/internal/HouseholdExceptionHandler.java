...
class HouseholdExceptionHandler {
    ...
    ProblemDetail handleAlreadyInitialized(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "家庭已初始化", "HOUSEHOLD_ALREADY_INITIALIZED");
    }

    @ExceptionHandler(InvalidCredentialsException.class)
    ProblemDetail handleInvalidCredentials(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.UNAUTHORIZED, "未认证或账户未激活", "HOUSEHOLD_MEMBER_NOT_ACTIVE");
    }
