...
class CatalogExceptionHandler {
    ...
    ProblemDetail handlePrecisionInvalid(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.UNPROCESSABLE_CONTENT, "阈值精度超过单位允许范围", "CATALOG_UNIT_PRECISION_INVALID");
    }
    ...
    ProblemDetail handleArchivedDictionary(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "归档的字典项不可使用", "CATALOG_ARCHIVED_DICTIONARY");
    }

    @ExceptionHandler(CatalogCycleDetectedException.class)
    ProblemDetail handleCycleDetected(HttpServletRequest request) {
        return ZijaProblems.of(request, HttpStatus.CONFLICT, "移动会产生循环引用", "CATALOG_CYCLE_DETECTED");
    }
}
