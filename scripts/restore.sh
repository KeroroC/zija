...
for i in $(seq 1 60); do
  HTTP_CODE=$(curl -s -o /dev/null -w '%{http_code}' \
    "http://127.0.0.1:${HTTP_PORT}/actuator/health" 2>/dev/null || echo "000")
...
curl -sf -c "$COOKIES" -X POST "http://127.0.0.1:${HTTP_PORT}/api/v1/auth/login" \
  -H "Content-Type: application/json" \
  -d "{\"username\":\"owner@test.com\",\"password\":\"${OWNER_PASS}\"}" \
